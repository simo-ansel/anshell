 ...
 *  File: script_videogiochi.js
 *
 *  Gestione:
 *  - caricamento videogiochi
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    videogiochiData = await fetchOpere({ fileName: "videogiochi.json" });
    filteredData = videogiochiData;

    populateGenereChips(chipsContainer, videogiochiData, filters, applyFilters);
...
function applyFilters() {
    filteredData = videogiochiData.filter(item =>
...
async function renderGrid(data) {
    ...
    if (!data.length) {
        gridOpere.innerHTML = `<p>Nessun videogioco trovato.</p>`;
        return;
    }
