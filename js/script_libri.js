...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    libriData = await fetchOpere({ fileName: "libri.json" });
    filteredData = libriData;

    populateGenereChips(chipsContainer, libriData, filters, applyFilters);
...
function applyFilters() {
    filteredData = libriData.filter(item =>
...
async function renderGrid(data) {
    ...
    if (!data.length) {
        gridOpere.innerHTML = `<p>Nessun libro trovato.</p>`;
        return;
    }
