...
import {
    gridOpere, // Riferimento alla griglia dove verranno mostrate le opere (serie TV)
} from './functions.js'
 ...
 * Descrizione: Script per caricare i dati su serie TV da un file JSON e visualizzarli in una galleria interattiva.
 *              Ogni serie TV è cliccabile e apre un modale che permette di esplorare i dettagli.
 */

// Il codice viene eseguito quando il DOM è completamente caricato
document.addEventListener("DOMContentLoaded", async () => {

    // Recupera i dati dal file JSON specificato (in questo caso "serieTV.json")
    const serieTV = await fetchOpere({
        fileName: "serieTV.json" // Specifica il file da cui caricare i dati delle opere (serie TV)
    });
    console.log("Serie TV caricate:", serieTV); // Log per verificare i dati caricati correttamente

    // *****************************************************************************************
    // PROCESSO PRINCIPALE: Carica le opere, visualizza la galleria e aggiungi l'interattività
    // *****************************************************************************************

    if (serieTV.length > 0) { // Se ci sono serie TV disponibili

        // Crea le carte HTML per ciascuna opera (serie TV)
        const operaCards = await Promise.all(serieTV.map(createOperaCard));
        ...
        // Se non ci sono opere (serie TV) disponibili, mostra un messaggio di errore
