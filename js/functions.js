...
function formatPeriodo(periodo) {
    if (!periodo || !periodo.inizio) return "Data non disponibile";

    if (periodo.fine === null) {
        return `${periodo.inizio} – in corso`;
    }

    if (periodo.inizio === periodo.fine) {
        return `${periodo.inizio}`;
    }
...
export function showModal(opera) {
    ...
    const anno = createParagraph(
        "Anno",
        formatPeriodo(opera.periodo)
    );

    const genere = createParagraph(
        "Genere",
        (opera.genere || []).join(" | ") || "Genere non disponibile"
    );

    const valutazione = createParagraph("Valutazione");
...
export function matchesSearch(item, query) {
    ...
    return (
        ...
        item.genere.some(g => g.toLowerCase().includes(q))
    );
}


export function matchesGenere(item, genere) {
    if (genere === "all") return true;
    return item.genere.includes(genere);
}

/**
 * Verifica se un'opera corrisponde ai generi selezionati (multi-selezione tramite chip).
 * Se nessun genere è selezionato, l'opera viene considerata sempre valida (nessun filtro attivo).
 * Un'opera è considerata valida se possiede ALMENO UNO dei generi selezionati.
 *
 * @param {Object} item - L'opera da verificare.
 * @param {Set<string>} generiSelezionati - Insieme dei generi attualmente selezionati.
 *
 * @returns {boolean} True se l'opera corrisponde ai generi selezionati (o se non ci sono filtri attivi).
 */
export function matchesGenereMulti(item, generiSelezionati) {
    if (!generiSelezionati || generiSelezionati.size === 0) return true;
    return (item.genere || []).some(g => generiSelezionati.has(g));
}

// ***********************************************************************
// FUNZIONE: Popola e gestisce i "chip" (pulsanti) per il filtro dei generi
// ***********************************************************************

/**
 * Genera dinamicamente i chip cliccabili per il filtro dei generi (multi-selezione),
 * inclusi un chip "Tutti" per azzerare la selezione. Gestisce anche lo stato attivo/inattivo
 * dei chip e invoca la callback fornita ogni volta che la selezione cambia.
 *
 * @param {HTMLElement} chipsContainer - Il contenitore dove inserire i chip dei generi.
 * @param {Array} data - L'array di opere da cui estrarre i generi disponibili.
 * @param {Object} filters - L'oggetto di stato dei filtri, con una proprietà `generi` (Set).
 * @param {Function} onChange - Callback invocata ogni volta che la selezione dei generi cambia.
 */
export function populateGenereChips(chipsContainer, data, filters, onChange) {
    if (!chipsContainer) return;

    chipsContainer.innerHTML = "";

    // Raccoglie tutti i generi unici presenti nei dati
    const generi = new Set();
    data.forEach(item => (item.genere || []).forEach(g => generi.add(g)));

    // Se non ci sono generi disponibili, nasconde il contenitore
    if (generi.size === 0) {
    ...
    // Chip speciale "Tutti": azzera la selezione dei generi
    const chipTutti = document.createElement("button");
    chipTutti.type = "button";
    chipTutti.className = "genre-chip genre-chip-all active";
    chipTutti.textContent = "Tutti";
    chipTutti.setAttribute("aria-pressed", "true");

    const chipButtons = [];

    function updateChipsActiveState() {
        const nessunGenereAttivo = filters.generi.size === 0;
        chipTutti.classList.toggle("active", nessunGenereAttivo);
        chipTutti.setAttribute("aria-pressed", String(nessunGenereAttivo));

        chipButtons.forEach(chip => {
            const attivo = filters.generi.has(chip.dataset.genere);
            chip.classList.toggle("active", attivo);
            chip.setAttribute("aria-pressed", String(attivo));
        });
    }

    chipTutti.addEventListener("click", () => {
        filters.generi.clear();
        updateChipsActiveState();
        onChange();
    });
    chipsContainer.appendChild(chipTutti);

    // Chip per ogni genere disponibile, ordinati alfabeticamente
    [...generi].sort((a, b) => a.localeCompare(b)).forEach(genere => {
        const chip = document.createElement("button");
        chip.type = "button";
        chip.className = "genre-chip";
        chip.textContent = genere;
        chip.dataset.genere = genere;
        chip.setAttribute("aria-pressed", "false");

        chip.addEventListener("click", () => {
            if (filters.generi.has(genere)) {
                filters.generi.delete(genere);
            } else {
                filters.generi.add(genere);
            }
            updateChipsActiveState();
            onChange();
        });
...
// ***********************************************************************
// FUNZIONE: Ripristina lo stato iniziale dei filtri (ricerca, generi, ordine)
// ***********************************************************************

/**
 * Ripristina i filtri allo stato iniziale: svuota la ricerca testuale, deseleziona
 * tutti i generi e riporta l'ordinamento al valore predefinito. Aggiorna anche
 * gli elementi del DOM corrispondenti (input, select, chip).
 *
 * @param {Object} filters - L'oggetto di stato dei filtri da resettare.
 * @param {Object} elements - Riferimenti agli elementi del DOM: { searchInput, ordineSelect, chipsContainer }.
 */
export function resetFiltri(filters, elements) {
    const { searchInput, ordineSelect, chipsContainer } = elements;

    filters.search = "";
    filters.generi.clear();
    filters.ordine = "titolo-asc";

    if (searchInput) searchInput.value = "";
    if (ordineSelect) ordineSelect.value = "titolo-asc";

    if (chipsContainer) {
        chipsContainer.querySelectorAll(".genre-chip").forEach(chip => {
            const isTutti = chip.classList.contains("genre-chip-all");
            chip.classList.toggle("active", isTutti);
            chip.setAttribute("aria-pressed", String(isTutti));
        });
    }
}

export function matchesAutore(item, autore) {
    if (autore === "all") return true;
    return item.autore === autore;
}

export function matchesAnno(item, anno) {
    if (anno === "all") return true;

    const y = Number(anno);
    const { inizio, fine } = item.periodo;

    return inizio <= y && (fine === null || fine >= y);
}
...
export function sortItems(items, order) {
    ...
    switch (order) {
        ...
        case "anno-asc":
            return sorted.sort((a, b) =>
                a.periodo.inizio - b.periodo.inizio
            );

        case "anno-desc":
            return sorted.sort((a, b) =>
                b.periodo.inizio - a.periodo.inizio
            );
