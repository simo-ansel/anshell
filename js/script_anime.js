...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    animeData = await fetchOpere({ fileName: "anime.json" });
    filteredData = animeData;

    populateGenereChips(chipsContainer, animeData, filters, applyFilters);
...
async function renderGrid(data) {
    ...
    if (!data.length) {
        gridOpere.innerHTML = `<p>Nessun anime trovato.</p>`;
        return;
    }
