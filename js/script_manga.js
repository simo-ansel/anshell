...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    mangaData = await fetchOpere({ fileName: "manga.json" });
    filteredData = mangaData;

    populateGenereChips(chipsContainer, mangaData, filters, applyFilters);
...
function applyFilters() {
    filteredData = mangaData.filter(item =>
...
async function renderGrid(data) {
    ...
    if (!data.length) {
        gridOpere.innerHTML = `<p>Nessun manga trovato.</p>`;
        return;
    }
